                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control" name="rev_no" id="rev_no_edit" required placeholder="Rev No.">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    function EditDar(url){
        ...
        success: function(response) {
            ...
            if (response.data[0].number_doc) {
                $('.modal-title.edit').text(`EDIT REQUEST DAR - ${response.data[0].number_doc}`);
            }
            $('#id-reqdar-form').val(response.data[0].reqdar_id);

            // Set requester information
            $('#by_add').text(`Requested by: ${response.data[0].name || 'N/A'}`);

            // Populate form fields with data
            // Type of request
            if (response.data[0].typereqform_id) {
                $(`#type_${response.data[0].typereqform_id}_edit`).prop('checked', true);
            }

            // Request description
            if (response.data[0].request_desc_id) {
                $(`#request-desc-id_${response.data[0].request_desc_id}_edit`).prop('checked', true);
            }

            // Department
            $('#dept-id-edit').val(response.data[0].dept_id);

            // Document information
            $('#name-doc-edit').val(response.data[0].name_doc);
            $('#no-doc-edit').val(response.data[0].no_doc);
            $('#qty-pages-edit').val(response.data[0].qty_pages);
            $('#reason-edit').val(response.data[0].reason);
            $('#rev_no_edit').val(response.data[0].rev_no);

            // APPROVAL 1
            $('#approved-by1-edit').text(response.data[0].approval_by1);
            $('#approval-date1-edit').text(response.data[0].approval_date1);
            let approvalStatus1 = response.data[0].approval_status1 == '0' ? 'Waiting Approval' :
                                response.data[0].approval_status1 == '1' ? 'Approved' :
                                response.data[0].approval_status1 == '2' ? 'Rejected' :
                                'Unknown';
            $('#approval-status1-edit').text(approvalStatus1);
            $('#remark-approval1-edit').text(response.data[0].remark_approval_by1);
            // APPROVAL 2
            $('#approved-by2-edit').text(response.data[0].approval_by2);
            $('#approval-date2-edit').text(response.data[0].approval_date2);
            let approvalStatus2 = response.data[0].approval_status2 == '0' ? 'Waiting Approval' :
                                response.data[0].approval_status2 == '1' ? 'Approved' :
                                response.data[0].approval_status2 == '2' ? 'Rejected' :
                                'Unknown';
            $('#approval-status2-edit').text(approvalStatus2);
            $('#remark-approval2-edit').text(response.data[0].remark_approval_by2);
            // APPROVAL 3
            $('#approved-by3-edit').text(response.data[0].approval_by3);
            $('#approval-date3-edit').text(response.data[0].approval_date3);
            let approvalStatus3 = response.data[0].approval_status3 == '0' ? 'Waiting Approval' :
                                response.data[0].approval_status3 == '1' ? 'Approved' :
                                response.data[0].approval_status3 == '2' ? 'Rejected' :
                                'Unknown';
            $('#approval-status3-edit').text(approvalStatus3);
            $('#remark-approval3-edit').text(response.data[0].remark_approval_by3);

            $('#file-storage').val(response.data[0].file_doc);
            if (response.data[0].file_doc) {
                    const fileName = response.data[0].file_doc.split('/').pop();
                    $('.custom-file-label-edit').text(fileName);
                    $('#pdf-file-name').text(fileName);
                    $('#file-doc-path').val(response.data[0].file_doc);
                    
                    // Enable tombol view PDF dan set document ID
                    $('#view-pdf-btn').data('document-id', response.data[0].reqdar_id);
                ...
                // Storage type
            if (response.data[0].storage_type === 'month') {
                $('#storage_type_month_edit').prop('checked', true);
            } else if (response.data[0].storage_type === 'year') {
                $('#storage_type_year_edit').prop('checked', true);
            }

            if (response.data[0].file_doc) {
                $('.custom-file-label-edit').text(response.data[0].file_doc.split('/').pop());
            }
            let currentPdfPath = '';
            ...
            updateApprovalStatus(response.data[0]);
    ...
    function updateApprovalCard(level, data) {
      ...
      function formatDate(dateString) {
        ...
        $(document).on('click','.editfrm', function(){
            if (!validateFileSize('file_doc_edit', 5)) {
                return false;
            }
    ...
    document.getElementById('file_doc_edit').addEventListener('change', function() {
        validateFileSize('file_doc_edit', 5); // 5MB max
    });
